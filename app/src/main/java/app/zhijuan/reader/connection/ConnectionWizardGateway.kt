...
import app.zhijuan.provider.anthropic.AnthropicMessagesAdapter
import app.zhijuan.provider.anthropic.RegistryBackedAnthropicCapabilityResolver
...
import app.zhijuan.provider.gemini.GeminiGenerateContentAdapter
import app.zhijuan.provider.gemini.RegistryBackedGeminiCapabilityResolver
...
import app.zhijuan.provider.openai.responses.OpenAiResponsesAdapter
import app.zhijuan.provider.openai.responses.RegistryBackedOpenAiResponsesCapabilityResolver
...
class ConnectionWizardGateway @Inject constructor(
            ...
            adapters = ProviderAdapterRegistry(
                listOf(
                    OpenAiResponsesAdapter(
                        transport = transport,
                        capabilityResolver = RegistryBackedOpenAiResponsesCapabilityResolver(capabilityRegistry),
                    ),
                    OpenAiChatAdapter(
                        transport = transport,
                    ...
                    AnthropicMessagesAdapter(
                        transport = transport,
                        capabilityResolver = RegistryBackedAnthropicCapabilityResolver(capabilityRegistry),
                    ),
                    GeminiGenerateContentAdapter(
                        transport = transport,
                        capabilityResolver = RegistryBackedGeminiCapabilityResolver(capabilityRegistry),
                    ),
                ),
            ),
    ...
    private fun endpointFor(input: ConnectionWizardInput): String? = when (input.service) {
        ...
        ConnectionServiceChoice.ANTHROPIC -> ANTHROPIC_BASE_URL
        ConnectionServiceChoice.GEMINI -> GEMINI_BASE_URL
    ...
    private fun protocolFor(service: ConnectionServiceChoice): ProviderProtocol = when (service) {
        ConnectionServiceChoice.OPENAI -> ProviderProtocol.OPENAI_RESPONSES
        ConnectionServiceChoice.DEEPSEEK,
        ConnectionServiceChoice.RELAY,
        -> ProviderProtocol.OPENAI_CHAT_COMPAT
        ConnectionServiceChoice.ANTHROPIC -> ProviderProtocol.ANTHROPIC_MESSAGES
        ConnectionServiceChoice.GEMINI -> ProviderProtocol.GEMINI_GENERATE_CONTENT
    }
    ...
    private fun recommendModel(
        ...
        val preferredMarkers = when (service) {
            ...
            ConnectionServiceChoice.ANTHROPIC -> listOf("claude-sonnet", "claude")
            ConnectionServiceChoice.GEMINI -> listOf("gemini-pro", "gemini")
    ...
    private fun defaultDisplayName(service: ConnectionServiceChoice, endpointHost: String): String = when (service) {
        ...
        ConnectionServiceChoice.ANTHROPIC -> "Anthropic"
        ConnectionServiceChoice.GEMINI -> "Gemini"
