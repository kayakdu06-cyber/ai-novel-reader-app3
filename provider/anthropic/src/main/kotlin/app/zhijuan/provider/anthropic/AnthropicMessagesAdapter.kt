...
class AnthropicMessagesAdapter(
    ...
    private val capabilityResolver: AnthropicCapabilityResolver = ConservativeAnthropicCapabilityResolver(),
    private val clock: () -> Long = System::currentTimeMillis,
    private val connectionTestTimeouts: ProviderTimeoutPolicy = DEFAULT_CONNECTION_TIMEOUTS,
) : ProviderAdapter {
    override val protocol = ProviderProtocol.ANTHROPIC_MESSAGES
    ...
    override suspend fun listModels(profile: ProviderConnectionProfile): ModelListResult = withContext(Dispatchers.IO) {
        ...
        try {
            val spec = ProviderHttpRequestSpec(
                requestId = "anthropic-models-" + auxiliarySequence.incrementAndGet(),
                profile = profile,
    ...
    override fun generate(
        ...
        try {
            if (profile.protocol != protocol) throw AnthropicProtocolException()
            ...
            val body = AnthropicRequestEncoder.encode(profile, request, capabilities)
            val spec = ProviderHttpRequestSpec(
                requestId = request.requestId,
                profile = profile,
                method = ProviderHttpMethod.POST,
                pathSegments = listOf("messages"),
    ...
    private suspend fun FlowCollector<ProviderStreamEvent>.handleResponse(
        ...
        if (response.statusCode !in 200..299) {
            ...
            val error = try { parseAnthropicErrorObject(bytes) } finally { bytes.fill(0) }
            emitAccepted(
                gate,
                ProviderStreamEvent.Failed(
                    code = AnthropicErrorMapper.map(response.statusCode, error),
        ...
        when {
            ...
            (!requestedStreaming && !isSse && (isJson || contentType == null)) ||
                ...
                val bytes = response.withBodySourceSuspending { it.readByteArray() }
                val root = try { AnthropicJson.objectFrom(bytes) } finally { bytes.fill(0) }
                AnthropicNonStreamingMapper.map(root, structuredOutput).forEach {
    ...
    private suspend fun FlowCollector<ProviderStreamEvent>.consumeSse(
        ...
        val mapper = AnthropicStreamMapper(structuredOutput)
            ...
            try {
                if (response.statusCode !in 200..299) {
                    ModelListResult.Failure(
                        ProviderCallFailure(
                            code = AnthropicErrorMapper.map(response.statusCode, parseAnthropicErrorObject(bytes)),
                            httpStatus = response.statusCode,
                            retryAfterMillis = retryAfter,
                            requestState = FailureRequestState.PROVIDER_REJECTED,
                        ),
                    ...
                    val models = AnthropicModelListMapper.map(AnthropicJson.objectFrom(bytes)).mapNotNull {
                        runCatching { ProviderModelSummary(ProviderModelId.from(it)) }.getOrNull()
                    }
    ...
    private fun apiHeaders(accept: String) = mapOf(
        "Accept" to accept,
        "anthropic-version" to API_VERSION,
    )

    private fun primarySecretHeader(profile: ProviderConnectionProfile): PrimarySecretHeader? =
        profile.primarySecretRefId?.let { PrimarySecretHeader("x-api-key", PrimarySecretScheme.RAW) }

    private fun remoteRequestId(response: ProviderHttpResponseLease): ProviderRemoteRequestId? =
        response.withHeaderValue("request-id") { it }?.let {
            runCatching { ProviderRemoteRequestId.from(it) }.getOrNull()
        }
    ...
    companion object {
        ...
        private const val ADAPTER_VERSION = "anthropic-messages-1"
        private const val MAXIMUM_MODEL_LIST_BYTES = 2L * 1024 * 1024
        private const val MAXIMUM_GENERATION_RESPONSE_BYTES = 64L * 1024 * 1024
        private const val STREAM_READ_BUFFER_BYTES = 8 * 1024
        private val DEFAULT_CONNECTION_TIMEOUTS = ProviderTimeoutPolicy(5_000, 15_000, 15_000, 60_000)
    }
}
