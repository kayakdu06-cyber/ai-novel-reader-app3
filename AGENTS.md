...
- 本仓库只对应 Codex 任务“app开发3”。
- 唯一允许修改的项目根目录是 `D:\gptuser\projects\ai-novel-reader-app3`。
...
- 本副本保留 app开发2 的 Git 历史，以提交 `89ec64da146b27d6186ef71c32c5b1a565e2f52c` 为迁移基线，并叠加 `ai-novel-reader-slim (2).zip` 后按 `MODULARIZATION-PLAN.md` 独立整改。不得把 app开发2 目录设置为 remote，也不得向 app开发2 写入任何内容。
...
- 用户已授权 Sol 在 app开发3 的开发过程中自行决定何时调用项目隔离的 DeepSeek V4 Flash 编码模型，无需逐次确认；每次交接应在进度更新中简短说明。此授权不包含调用“织卷”App 内部的真实生成接口、接入新的付费服务或扩大到其他项目。不得把密钥写入文档/日志/版本库，也不得由 DeepSeek 把任务状态改成“完成”。完成状态必须由 Sol 在审查差异并取得相应测试证据后确认。
