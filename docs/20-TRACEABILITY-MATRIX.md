...
|---|---|---|
| 示例被误读成只支持修仙、恋爱、系统等类型 | DEC-084；PR-003；27 §6.2；预设、能力 ID 和样本一律注明非白名单 | TASK-122 用一个未列入预设的自由题材证明原文进入 Prompt |
| 未命中预设后自由输入被丢弃、拒绝或替换成默认题材 | 创建快照保留原始输入；通用叙事语义承接没有专用适配器的内容 | 同一 TASK-122 用例断言原文保留、无关适配器零占用 |
| 为支持开放题材而给每种类型新增代码、表或测试 | 专用适配器只服务确定性长期状态；新增必须有实际可靠性缺陷证据 | 任务范围审查；没有故障链不得扩展模块、migration 或样本矩阵 |
| 开放输入被误实现成运行时插件或 skill 安装 | 用户文本仅作为数据；继续禁止动态 capability、可执行插件和任意 skill 导入 | TASK-121/122 合同审查，不新增运行时 registry 或导入 UI |
