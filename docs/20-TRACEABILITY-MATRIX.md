...
| 真实 API 拖到最后才暴露问题 | 31 号分级 R0～R3；TASK-132/133 | DeepSeek V4 Flash 脱敏报告 |
| 代码存在但书架/阅读/模板无生产调用 | TASK-131、134 | VS-TEST-008、模板三步重开验收 |
| 旧计划与新顺序冲突 | 27/30 号文档权威；DEC-076 | CURRENT-CONTEXT、README、Git 提交 |

## 59. 2026-08-11 模块、范围和精简测试约束

| 用户要求/风险 | 执行约束 | 验证证据 |
|---|---|---|
| 严格按模块开发 | DEC-081；每任务声明主模块/配套模块；VS-1 冻结十模块 | `verify-module-boundaries.ps1`、任务允许文件清单、Git diff |
| app 壳重新堆积业务 | `:app` 只允许导航/Hilt/最小映射 | module review；业务类不得出现在 app main |
| 主动扩展无用功能 | DEC-082；没有严重故障链就只记录不实现 | 任务包“非目标”和扩展证据审查 |
| 重复测试浪费时间/token | DEC-083；按风险选择最小测试 | 每项测试关联具体故障；无重复等价场景 |
| 真实 API 测试过多 | 31 号文档 R0～R3 最小序列；A/B 条件触发 | 一个结构 smoke、一个完整混合章、同书 3–5 章报告 |

## 60. 2026-08-11 开放题材与状态适配器边界

| 用户要求/风险 | 设计与执行约束 | 最小验证 |
|---|---|---|
| 创作类型不受预设、能力 ID 或测试样本限制 | DEC-084；PR-003；27 §6.2；用户自由描述是权威创作意图 | TASK-122 用一个未列入预设的自由题材证明原文进入 Prompt |
| 自由输入必须完整进入规划 | 创建快照保留原始输入；通用叙事语义承接没有专用适配器的内容 | 同一 TASK-122 用例断言原文保留、无关适配器零占用 |
| 专用适配器只服务确定性长期状态 | 新增适配器必须有实际可靠性缺陷证据 | 任务范围审查；没有故障链不得扩展模块、migration 或样本矩阵 |
| 开放输入只是数据，不是可执行扩展 | 继续禁止动态 capability、可执行插件和任意 skill 导入 | TASK-121/122 合同审查，不新增运行时 registry 或导入 UI |

## 61. 2026-08-11 真实试写验收口径

| 产品要求 | 执行约束 | 最小验证 |
|---|---|---|
| 使用最高敏感度题材验证真实返回 | R2/R3 固定为“细写”档、明确虚构成年人、事件推动主线并形成后续状态 | sensitiveSceneContractCoverage=100%，淡出/跳过/事后概述替代为 0 |
| 文笔不属于测试门槛 | 不评辞藻、修辞、文风美感、感染力或继续阅读意愿 | 报告只含通过/不通过逻辑清单，不含文笔分数 |
| 只看小说逻辑是否可靠 | 检查计划兑现、因果、义务、人物知识、关系、道具、机制、时间线、重复和后果 | R2 完整章 + 同书 R3 连续 3–5 章 |
| Provider 拒绝不能伪装成通过 | 如实记录拒绝，不降低档位或改写意图循环规避 | 报告标记该模型/端点不满足目标，App 不提交假正文 |
