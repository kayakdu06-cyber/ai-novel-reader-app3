...
- TASK-130 已完成：费用确认页会复核冻结快照、当前连接/模型、目的地和三层预算上限，幂等创建唯一入口 Job；同一持久 runner 可依次提交初始规划、首个 8 章窗口并准备第一章，随后进入生成中页。失败保留确认页并允许重试，真实 Provider 测试调用 0。
- 唯一下一任务是 TASK-131：按模块分批接通最小书架、目录、正式正文与生成中只读投影。
...
- TASK-131：接通最小书架、目录、正式正文阅读、生成中只读投影和后台生成状态。
- 必须分 `:feature:library`、`:feature:reader`、`:app` 批次提交；Compose 不依赖 data 实现，不新增 feature 实现依赖。
- 复用 TASK-129/130 的唯一 runner、正式正文版本和生成状态；不提前进行 TASK-132 的真实 Provider 测试。
