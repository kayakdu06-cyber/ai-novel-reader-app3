...
- [ ] `chapter-post-analysis.v1` 严格合同与 parser 完成。
- [ ] 混合正例覆盖义务、系统、道具、关系及正文证据。
- [ ] 一个子块非法时整体拒绝且没有部分状态写入。
- [ ] 严重重复不提交，生成有限修订谱系，修订后重新分析。
- [ ] 正常路径章后远程调用数不超过 1，并写入计时报告。
- [ ] 现有最终章节原子提交复用成功。
- [ ] 相关 JVM/Android 测试、边界检查、安全扫描、`assembleDebug`、`test` 通过。
- [ ] 每个完成点单独提交并推送 Git。
