...
- 原因：当前 10 模块和约 60,100 行 Kotlin 已覆盖大量底层，App 仍停在费用确认占位，书架、阅读和生成启动未接通。继续水平扩展不能验证小说逻辑、敏感内容合同和真实速度。
...
- 状态：已确认，2026-08-11
- 决策：一本书可以同时启用多种创作意图；修仙、恋爱、系统、道具、悬疑和亲密连续性等名称仅为示例，不是能力白名单。每章只激活当前相关的专用状态适配器；各适配器共享 NarrativeObligation 与 StoryStateDelta 外壳，并拥有自己的合法状态转移规则。
...
- 原因：用户需要可靠可用，但最关键的小说逻辑、敏感内容合同、速度和交互只能通过真实闭环验证。过晚测试会放大返工。
...
## DEC-084 题材词汇开放，状态适配器不是类型白名单

- 状态：已确认，2026-08-11
- 决策：用户可用自由文本描述并组合任意题材、背景、关系、故事机制、叙事结构和文风。预设、能力 ID、文档示例和测试样本都不能作为支持白名单。BookCapabilityManifest 只选择需要确定性跨章约束的内部状态适配器；没有专用适配器的内容继续由通用叙事、人物连续性、叙事义务和证据语义处理。
- 原因：把示例当白名单会丢失或拒绝用户意图；反过来为每个题材开发专用代码又会扩大模块、Prompt、测试和维护成本。
- 后果：原始自由描述必须保存并进入规划，未命中预设不得静默改成默认题材。只有“任务明确要求某个专用适配器但其 ID/版本不可用”才失败关闭；未知题材文本本身不是错误。开放输入不引入动态插件、运行时 skill 安装或用户可执行 capability。新增适配器必须由真实的长期状态可靠性缺陷驱动。

## DEC-085 真实试写使用最高敏感度样本且只验小说逻辑

- 状态：已确认，2026-08-11
- 决策：真实模型 R2/R3 固定使用“细写”档、明确虚构成年人的最高敏感度剧情，并要求相关事件推动主线并形成后续关系、义务或状态变化。Provider 拒绝、淡出、跳过或事后概述替代均不能记为达标。评测不包含文笔、辞藻、文风美感、文学感染力或继续阅读意愿。
- 原因：最高敏感度场景同时暴露模型拒绝、场景合同执行、身体/因果连续性和关系后果遗漏；文笔偏好主观且不能证明长篇状态架构可靠。
- 后果：真实报告只给小说逻辑、敏感场景合同、状态连续性、重复、性能、费用和拒绝结果。测试不降低档位或循环改写意图规避 Provider；失败后按协议、合同、Prompt、状态或拒绝归因修复。
