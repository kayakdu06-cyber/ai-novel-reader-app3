...
1. **严格按现有十模块开发**
   - VS-1 前不得新增第十一个模块。
   - 每个任务包必须写明“主模块、允许的配套模块、禁止模块和依赖方向”。
   - `:core` 只放纯 Kotlin 公共模型/契约；`:data` 只放存储、安全、诊断和既有持久事务；`:provider` 只放 Provider/transport；业务编排属于 `:feature:generation`；页面属于对应 feature；`:app` 只做导航和 Hilt 组装。
   - feature 之间不得新增实现依赖；现有唯一例外仍是 `:feature:template -> :feature:creation`。
   - 跨模块功能按契约、实现、持久化、壳接线分批修改；每个模块批次独立审查、提交和推送后再进入下一批。不得把业务逻辑塞进 `:app`，也不得用 sourceSets 指向其他模块。
   - 不因“顺手整理”迁移现有代码；只有当前放置直接导致依赖环、数据污染、崩溃或无法修复的严重问题时，才允许立项调整。

2. **禁止主动扩展**
   - 任务只实现当前验收所必需的最小路径。
   - 不新增插件系统、通用扩展点、可选 Provider、设置项、管理页、抽象层、数据库表或兼容层，除非缺少它会直接造成 App 不稳定、数据损坏、费用失控、安全问题或已确认严重 bug。
   - 发现“以后可能有用”的需求只记录到风险/候选清单，不在当前任务实现。
   - 扩大范围前必须给出直接故障链：缺少什么 → 会触发什么严重问题 → 为什么现有结构不能处理。没有这条证据就不扩展。

3. **按风险选择最小测试**
   - 文档改动：只做引用、格式、Git 差异和一致性检查。
   - 单模块纯逻辑：只跑该模块的相关测试与编译。
   - 跨模块契约：跑边界脚本、受影响模块编译和一个覆盖真实交接的集成测试。
   - 数据库迁移、安全、预算、Provider-open、runner 原子性：跑对应专项回归；只有这些高风险变化或里程碑才扩大验证。
   - `assembleDebug` 在形成可安装/可导航切片时运行；统一全量 `test`、Release/R8、双 Android API 和 APK 安全扫描只在里程碑、发布或相应平台风险变化时运行。
   - 不重复测试已经由同一层证据覆盖的等价场景；每个新增测试必须写明它保护的具体风险。
...
3. 运行任务相关的最小风险测试；只有触发 4.1 的高风险或里程碑条件时才运行统一离线门禁；
