...
需要生成 token 的真实测试只保留 R1、R2、R3 三组；R0 优先复用零生成能力证据，不为协议、流式、题材和质量分别重复付费请求。
...
- 样本包含修仙、恋爱、系统和道具，但本章只激活实际相关子集；
- 正常路径不主动制造修订、取消或断网。

通过：

- 第一可读段 P95 ≤25 秒；
- 正文 P95 ≤300 秒，正式提交 P95 ≤360 秒；
- 无乱码、重复 chunk 或丢尾；
- 正文、状态和 Usage 原子一致；
- 未激活能力不进入 Prompt；
- 本章有推进和后果，关键状态无严重矛盾。
...
2. 至少一个 3–5 章连续样本；
3. R2/R3 使用同一个组合能力样本；
4. 性能、质量和费用有报告；拒绝或限流若实际发生则如实记录，不为覆盖该分支主动付费造错；
5. 用户在物理设备上实际阅读；
6. secret/API 数据扫描通过；
7. 修复已进入 Git 并同步；
