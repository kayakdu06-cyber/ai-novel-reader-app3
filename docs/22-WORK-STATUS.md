...
- M1 本地骨架与连接：进行中
- M2 单章闭环：进行中（正文流、有限续接、章节记忆及时间线/伏笔投影已完成，检查、正式提交/UI 待接通）
...
| TASK-083 三层预算和原子预留 | 领域原型完成 | 请求/单书/每日三层硬限制与结算测试；数据库事务接入待后续任务 |
...
- 下一步进入 TASK-059：实现有限修订与最终提交门禁。每次修订必须建立新候选/hash，使旧记忆、追踪与检查失效并重新提取/复检；自动修订次数必须固定，最终正文、记忆、时间线/伏笔、检查报告、Usage、Stage/Job 需要原子提交。仍使用本地规则/假 Provider，不启用真实付费生成；物理设备保持只读。
...
- 当前 Gradle JVM 报告：396 项通过；按模块+类+用例名去除 `core:backup` 的 15 项 test/testDebugUnitTest 重跑后为 381 个唯一用例，0 失败、0 错误、0 跳过。
...
当前 APK 已有可操作的首次启动说明、连接向导、持久连接列表、带可选高级区和正式篇幅规则的创建页，以及从不可变快照回读的开始前确认占位页。Prompt Bundle v1、故事种子/圣经/总纲、分卷/8 章窗口、首章最小包、第二章硬闸门、章前上下文预算、章节正文/有限续接、章节记忆、时间线/伏笔投影和一致性检查底层链已经通过本地规则/假 Provider 验证，但确认页尚未把用户动作接到真实预算、目的地和 Provider 门禁。有限修订、候选正文最终原子提交、编辑后跨章顺序重建、自动补窗调度、边生成边阅读和模板界面尚未接通，不应当作可用产品分发。
