...
状态：完成（2026-08-12，仅 `:feature:generation`；开放创作意图、书级 Manifest、章级激活与最小 Prompt 选择通过 13 项模块测试）。
...
状态：已完成（2026-08-13）。已交付 `chapter-post-analysis.v1` 单次合并分析、严格整体校验、现有状态仓库映射、严重问题有限修订/重新分析，以及既有 Room 最终原子提交复用。正常正文后分析调用不超过 1 次；真实 Provider 调用 0。
...
状态：完成（2026-08-13，`:feature:generation` 有界章节循环、同书连续 ordinal、任意暂停点恢复和 4 章 Room + Fake 专项通过；真实 Provider 0）。
...
状态：完成（2026-08-13）。冻结确认、唯一入口 Job、初始规划、首窗、第一章准备和生成中导航已接通；JVM 197/197、API 35 相关用例 10/10、十模块边界、安全扫描及 `assembleDebug test` 通过，真实 Provider 0。
...
1. 最小结构化合同；
2. 一个“细写”档、虚构成年人最高敏感度的完整混合章节，同时覆盖流式正文、合并分析、原子提交、小说逻辑、敏感场景合同和速度。
...
TASK-131 必须按模块分批接通最小书架、目录、正式正文和生成中只读投影；不得提前进行真实 Provider 验收或阅读视觉精修。
