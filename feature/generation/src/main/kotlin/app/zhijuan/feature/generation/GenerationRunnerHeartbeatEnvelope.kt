...
/**
 * Keeps the already-acquired Job and current Stage leases alive while one Stage action is active.
 * The action remains the sole owner of Provider, Attempt and business-commit behavior; this class
 * only schedules the atomic Phase 1C heartbeat primitive and coordinates coroutine lifetime.
 */
class GenerationRunnerHeartbeatEnvelope internal constructor(
    ...
    constructor(
        ...
        productionDependencies(executionLeases, states, clock, heartbeatIntervalMillis),
    )

    /**
     * Runs [action] and emits no heartbeat after the action completes. A heartbeat failure cancels
     * an action that still owns the same persisted Stage. If an authoritative business transaction
     * already advanced the cursor or moved the Job into a non-running terminal/waiting state, the
     * old heartbeat is expected to stale-fail; the envelope stops heartbeats and awaits the action
     * instead of cancelling a commit that has already durably crossed its boundary.
     */
    suspend fun <T> run(
                    ...
                    GenerationRunnerHeartbeatSignal.HeartbeatDue -> {
                        try {
                            ...
                            try {
                                ...
                                // The original heartbeat failure is the runner-facing cause.
                            }
    ...
    private companion object {
        fun productionDependencies(
            ...
            require(heartbeatIntervalMillis > 0L) { "Runner heartbeat interval must be positive." }
            require(heartbeatIntervalMillis < GenerationLeasePolicy.DEFAULT_TIMEOUT_MILLIS) {
                "Runner heartbeat interval must remain below the lease timeout."
            }
            return GenerationRunnerHeartbeatEnvelopeDependencies(
                heartbeat = { identity, heartbeatAt ->
                    executionLeases.heartbeatCurrentExecutionLeases(
                        jobId = identity.jobId,
                        jobLeaseToken = identity.jobLeaseToken,
                        stageId = identity.stageId,
                        stageLeaseToken = identity.stageLeaseToken,
